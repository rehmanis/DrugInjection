    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (row <= worksheetWithInstruction.Dimension.End.Row)
            {
                ...
                methodName = funcParamsList.First();
        ...
        public void ReadRow(ExcelWorksheet ws, int row, object datatype)
        {
            ...
            if (datatype is List<string>)
            {
                for (int col = 1; col <= ws.Dimension.End.Column; col++)
                {
                    ...
                    {
                        ((List<string>)datatype).Add(ws.Cells[row, col].Value.ToString());
                    }
                }
            ...
            if (datatype is Dictionary<string, List<string>>)
            {
                if (ws.Cells[row, 1].Value != null)
                {
                    ...
                    if (!dict.ContainsKey(ws.Cells[row, 1].Value.ToString()))
                        dict.Add(ws.Cells[row, 1].Value.ToString(), new List<string>());
                    ...
                    {
                        ((Dictionary<string, List<string>>)datatype)[ws.Cells[row, 1].Value.ToString()].Add(value);
                    }
